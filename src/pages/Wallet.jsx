...
const Wallet = () => {
  ...
  if (loading) {
    return (
      <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', padding: '60px 20px', minHeight: '400px', background: 'var(--bg-primary, #090d16)', color: 'var(--accent-primary, #14b8a6)' }}>
        <div style={{ textAlign: 'center' }}>
          <i className="fa-solid fa-wallet" style={{ fontSize: '48px', animation: 'spin 2s linear infinite', display: 'block', marginBottom: '16px', color: 'var(--accent-primary, #14b8a6)' }} />
  ...
  return (
    <div className="wallet-container-modern">
      ...
      <WalletActions walletBalance={walletData.balance} availableBalance={availableBalance} lockedBalance={walletData.lockedBalance} onRefresh={handleRefresh} />
      ...
      <div className="filter-section">
        {FILTER_TYPES.map((type) => (
          ...
          >
            {type.label}
            {type.key === 'all' && transactions.length > 0 && <span className="filter-count">{transactions.length}</span>}
          </button>
        ))}
      ...
      <div className="transactions-list-modern">
        {filteredTransactions.length === 0 ? (
          ...
          filteredTransactions.slice(0, 10).map((tx) => {
            ...
            return (
              <div key={tx.id} className={`transaction-item-modern ${tx.status}`} onClick={() => handleTransactionClick(tx)}>
                ...
                <div className="tx-details">
                  <h4>
                    ...
                    {tx.isEscrow && (
                      <span style={{ fontSize: '10px', marginLeft: '6px', color: '#f59e0b', border: '1px solid #f59e0b60', borderRadius: '20px', padding: '1px 8px' }}>ESCROW</span>
                    )}
                  </h4>
                  <p className="tx-date">{tx.formattedDate}</p>
                  {tx.reference && <small className="tx-reference">Ref: {tx.reference}</small>}
                  {tx.description && <small className="tx-description">{tx.description}</small>}
                  {tx.bankDetails && (
                ...
                <div className="tx-right">
                  ...
                  <div className={`tx-status ${tx.status}`} style={{ color: statusConfig.color, background: statusConfig.bg }}>
