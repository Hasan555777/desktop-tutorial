...
import { sendMoneyTransferNotification } from './notificationHelper';
...
//
// IMPORTANT: this performs a plain Firestore query (getDocs) — it must
// NEVER be called from inside a runTransaction() callback. Transaction
// callbacks may retry on write conflicts, which would re-run this
// non-transactional query on every retry and break the transaction's
// atomicity guarantee. Always call this BEFORE starting a transaction.
// ============================================================
...
const SendMoney = () => {
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      // Pre-check whether the receiver already has a wallet, and pre-generate
      // a wallet ID for them if not — done BEFORE the transaction, since
      // generateWalletId() runs a non-transactional query internally (see
      // its doc comment above) and must not execute inside runTransaction's
      // retryable callback.
    ...
    return (
      ...
      >
        <div style={{ textAlign: 'center' }}>
          ...
          />
          <h2 style={{ color: 'var(--text-primary, #f1f5f9)', fontSize: '20px', fontWeight: '600', margin: '0 0 8px 0' }}>Loading Wallet...</h2>
    ...
    <div className="sendmoney-container">
      <div className="sendmoney-card">
        ...
        <div className="balance-display">
          ...
          {lockedBalance > 0 && (
            <div className="locked-hint" style={{ fontSize: '12px', color: '#f59e0b', marginTop: '4px', display: 'flex', alignItems: 'center', gap: '4px', justifyContent: 'center' }}>
              <i className="fa-solid fa-lock"></i> {formatMoney(lockedBalance)} locked in active deals
              <span style={{ fontSize: '10px', color: '#94a3b8', marginLeft: '4px' }}>(Total: {formatMoney(balance)})</span>
            </div>
          )}
        </div>
        ...
        <div className="input-group">
          ...
          <div className="receiver-input-group">
            ...
            />
            <button className="find-btn" onClick={findReceiver} disabled={searching || loading || !receiverId.trim()}>
          ...
          {receiverName && (
            <div className="receiver-found">
              ...
              {receiverWalletId && <span className="receiver-wallet-id">(Wallet: {receiverWalletId})</span>}
            </div>
          )}
        </div>
        ...
        <div className="preset-amounts">
          {[100, 500, 1000, 2000].map((preset) => (
            <button key={preset} className={`preset-btn ${Number(amount) === preset ? 'active' : ''}`} onClick={() => setAmount(String(preset))} disabled={loading || !receiverName || preset > availableBalance}>
              ৳{preset}
            </button>
          ))}
        </div>
        ...
        <div className="input-group">
          ...
          <textarea value={note} onChange={(e) => setNote(e.target.value)} placeholder="Add a note for the receiver..." rows="2" maxLength="100" disabled={loading} />
        ...
        <button className="send-btn" onClick={handleSend} disabled={loading || !receiverName || !amount || Number(amount) > availableBalance || Number(amount) < MINIMUM_AMOUNT}>
          {loading ? (
            <>
