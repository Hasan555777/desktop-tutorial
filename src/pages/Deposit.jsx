...
import { auth, db } from '@/firebase'; // ✅ storage সরান
import { 
  doc, 
  addDoc, 
  collection, 
...
// ❌ Firebase Storage ইম্পোর্ট সরান
...
// ============================================================
// ✅ Cloudinary upload function (প্রফাইল থেকে কপি)
// ============================================================
...
const Deposit = () => {
    ...
    try {
      ...
      if (!snapshot.empty) {
        const existing = snapshot.docs.some(doc => {
          const data = doc.data();
          return data.status === DEPOSIT_STATUS.PENDING || 
                 data.status === DEPOSIT_STATUS.APPROVED;
        });
      ...
      console.error('❌ Error checking duplicate TrxID:', error);
      return true;
    }
  };

  // ============================================================
  // ✅ Upload Receipt to Cloudinary (ONLY ONE VERSION!)
  // ============================================================
  const uploadReceiptToCloudinary = async () => {
    if (!receiptFile || method !== PAYMENT_METHOD.BANK_TRANSFER) {
      return null;
    }
    ...
    try {
      ...
      console.log('✅ Receipt uploaded to Cloudinary:', imageUrl);
      return imageUrl;
  ...
  const handleDeposit = async () => {
    ...
    try {
      ...
      // Check duplicate for mobile banking
      if (!isBankTransfer) {
      ...
      // ✅ Upload receipt to Cloudinary (FIXED!)
      let receiptUrl = null;
      if (isBankTransfer && receiptFile) {
        try {
          ...
          if (receiptUrl) {
            ...
            console.log('✅ Receipt URL saved to transaction:', receiptUrl);
          }
        } catch (uploadError) {
          console.error('⚠️ Receipt upload failed but deposit saved:', uploadError);
          // Continue with deposit even if receipt upload fails
        }
      }
      ...
      try {
        ...
        await sendWalletDepositNotification(
          userId,
          amountNum,
          displayTrxId,
          method,
    ...
    <div className="deposit-container">
      <div className="deposit-card">
        ...
        <div className="deposit-header">
          <h2><i className="fa-solid fa-circle-dollar" style={{ color: '#3b82f6' }}></i> Deposit Money</h2>
          <p>Add funds to your wallet</p>
        </div>

        {status && (
          <div className={`status-message ${loading ? 'processing' : 'success'}`} style={{
            padding: '12px 16px',
        ...
        <div className="input-group">
          <label><i className="fa-solid fa-wallet"></i> Amount (BDT)</label>
          <input
            type="number"
            value={amount}
        ...
        {isBankTransfer && (
          <>
            <div className="bank-details-section">
              <h4><i className="fa-solid fa-building-columns"></i> Bank Account Details</h4>
            ...
            <div className="input-group">
              <label><i className="fa-solid fa-user"></i> Your Account Holder Name</label>
              <input
                type="text"
            ...
            <div className="input-group">
              <label><i className="fa-solid fa-hashtag"></i> Your Account Number</label>
              <input
                type="text"
            ...
            <div className="input-group">
              <label><i className="fa-solid fa-university"></i> Your Bank Name</label>
              <input
                type="text"
                value={bankName}
            ...
            <div className="input-group">
              <label><i className="fa-solid fa-location-dot"></i> Your Bank Branch</label>
              <input
                type="text"
                value={bankBranch}
            ...
            <div className="input-group">
              <label><i className="fa-solid fa-image"></i> Upload Receipt/Screenshot</label>
        ...
        {!isBankTransfer && (
          <>
            <div className="input-group">
              <label><i className="fa-solid fa-phone"></i> Sender Number</label>
              <input
                type="tel"
                value={senderNumber}
                onChange={(e) => handleNumberInput(e, setSenderNumber)}
                placeholder="Enter 11-digit bKash/Nagad/Rocket number"
                maxLength="11"
                disabled={loading}
              />
              <span className="input-hint">
                <i className="fa-solid fa-info-circle"></i> 
                The 11-digit number you sent money from
              </span>
            </div>

            <div className="input-group">
              <label><i className="fa-solid fa-receipt"></i> Transaction ID (TrxID)</label>
              <input
                type="text"
                value={trxId}
                onChange={handleTrxIdInput}
                placeholder="Enter transaction ID from your mobile banking app"
                disabled={loading}
              />
              <span className="input-hint">
                <i className="fa-solid fa-info-circle"></i> 
                Find TrxID in your {method} app transaction history
              </span>
            </div>
          </>
        )}
        ...
        <div className="input-group">
          <label><i className="fa-solid fa-pen"></i> Note (Optional)</label>
          <textarea
            value={note}
        ...
        <div className="payment-instructions">
          ...
          <ol>
            {isBankTransfer ? (
              <>
                <li>Transfer money to our bank account: <strong>{methodConfig?.bankDetails?.bankName}</strong></li>
                <li>Account Name: <strong>{methodConfig?.bankDetails?.accountName}</strong></li>
                <li>Account Number: <strong>{methodConfig?.bankDetails?.accountNumber}</strong></li>
                <li>Take a <strong>screenshot</strong> of your transfer confirmation</li>
              ...
              <>
                <li>Send money to this number: <strong>{methodConfig?.number}</strong></li>
