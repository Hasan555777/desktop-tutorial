...
const Transactions = () => {
  ...
  if (loading) {
    return (
      <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', padding: '60px 20px', minHeight: '300px', background: 'var(--bg-primary, #090d16)', color: 'var(--accent-primary, #14b8a6)' }}>
        <div style={{ textAlign: 'center' }}>
          <i className="fa-solid fa-clock-rotate-left" style={{ fontSize: '48px', animation: 'spin 2s linear infinite', display: 'block', marginBottom: '16px', color: 'var(--accent-primary, #14b8a6)' }} />
          <h2 style={{ color: 'var(--text-primary, #f1f5f9)', fontSize: '20px', fontWeight: '600', margin: '0 0 8px 0' }}>Loading Transactions...</h2>
    ...
    <div className="transactions-container">
      <div className="transactions-wrapper">
        ...
        <div className="filter-section">
          <div className="filter-group">
            ...
            <div className="filter-buttons">
              ...
              <button className={`filter-btn ${filterType === TRANSACTION_TYPE.CREDIT ? 'active' : ''}`} onClick={() => setFilterType(TRANSACTION_TYPE.CREDIT)}>
                <i className="fa-solid fa-arrow-down"></i> Received
                <span className="filter-count">{transactions.filter((t) => t.type === TRANSACTION_TYPE.CREDIT || t.type === TRANSACTION_TYPE.DEPOSIT || t.type === TRANSACTION_TYPE.EARNING).length}</span>
              </button>
              <button className={`filter-btn ${filterType === TRANSACTION_TYPE.DEBIT ? 'active' : ''}`} onClick={() => setFilterType(TRANSACTION_TYPE.DEBIT)}>
                <i className="fa-solid fa-arrow-up"></i> Paid
                <span className="filter-count">{transactions.filter((t) => t.type === TRANSACTION_TYPE.DEBIT).length}</span>
              </button>
              <button className={`filter-btn ${filterType === TRANSACTION_TYPE.WITHDRAW ? 'active' : ''}`} onClick={() => setFilterType(TRANSACTION_TYPE.WITHDRAW)}>
                <i className="fa-solid fa-money-bill-transfer"></i> Withdrawn
                <span className="filter-count">{transactions.filter((t) => t.type === TRANSACTION_TYPE.WITHDRAW).length}</span>
              </button>
            </div>
          </div>
          ...
          <div className="filter-group">
            ...
            <div className="filter-buttons status-filters">
              ...
              <button className={`filter-btn ${filterStatus === TRANSACTION_STATUS.COMPLETED ? 'active' : ''}`} onClick={() => setFilterStatus(TRANSACTION_STATUS.COMPLETED)}>
                <i className="fa-solid fa-check-circle" style={{ color: '#10b981' }}></i> Completed
              </button>
              <button className={`filter-btn ${filterStatus === TRANSACTION_STATUS.PENDING ? 'active' : ''}`} onClick={() => setFilterStatus(TRANSACTION_STATUS.PENDING)}>
                <i className="fa-solid fa-clock" style={{ color: '#f59e0b' }}></i> Pending
              </button>
              <button className={`filter-btn ${filterStatus === TRANSACTION_STATUS.FAILED ? 'active' : ''}`} onClick={() => setFilterStatus(TRANSACTION_STATUS.FAILED)}>
        ...
        <div className="result-count">
          ...
          {filteredTransactions.length !== transactions.length && <span className="filtered-info">(filtered)</span>}
        </div>
        ...
        <div className="transactions-list">
          {filteredTransactions.length === 0 ? (
            ...
            filteredTransactions.map((tx) => {
              ...
              return (
                <div key={tx.id} className="transaction-card" onClick={() => handleTransactionClick(tx)}>
                  ...
                  <div className="tx-content">
                    ...
                    <div className="tx-footer">
                      <div className="tx-meta">
                        ...
                        <span className={`tx-status ${tx.status}`} style={{ color: statusConfig.color, background: statusConfig.bg }}>
