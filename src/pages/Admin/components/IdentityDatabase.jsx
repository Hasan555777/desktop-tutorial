    ...
    try {
      ...
      if (existing) {
        const confirm = window.confirm(
          `⚠️ এই আইডি নম্বরটি "${existing.fullName}" এর সাথে মিলছে!\n\nআপনি কি তবুও সংরক্ষণ করতে চান?`
        );
        if (!confirm) {
          setLoading(false);
          return;
...
const IdentityDatabase = () => {
  ...
  return (
    <div className="identity-database">
      {/* ── Header ── */}
      <div className="idb-header">
        <div className="idb-header-left">
          <h2>
            ...
            Identity Database
            <span className="idb-badge">{stats.total}</span>
            {stats.autoAdded > 0 && (
              <span className="idb-badge auto">🤖 {stats.autoAdded} Auto</span>
            )}
          </h2>
        </div>
        <div className="idb-header-right">
          <button className="idb-btn-export" onClick={exportCSV}>
            <i className="fa-solid fa-file-export"></i> Export CSV
          </button>
          <button className="idb-btn-add" onClick={() => { setEditingRecord(null); setShowForm(true); }}>
      ...
      <div className="idb-stats">
        <div className="idb-stat">
          <span className="idb-stat-value">{stats.total}</span>
          <span className="idb-stat-label">Total</span>
        </div>
        <div className="idb-stat nid">
          <span className="idb-stat-value">{stats.nid}</span>
          <span className="idb-stat-label">🪪 NID</span>
        </div>
        <div className="idb-stat birth">
          <span className="idb-stat-value">{stats.birth}</span>
          <span className="idb-stat-label">📄 Birth</span>
        </div>
        <div className="idb-stat passport">
          <span className="idb-stat-value">{stats.passport}</span>
          <span className="idb-stat-label">🛂 Passport</span>
        </div>
        <div className="idb-stat pending">
          <span className="idb-stat-value">{stats.pending}</span>
          <span className="idb-stat-label">⏳ Pending</span>
        </div>
        <div className="idb-stat approved">
          <span className="idb-stat-value">{stats.approved}</span>
          <span className="idb-stat-label">✅ Approved</span>
        </div>
        <div className="idb-stat duplicate">
          <span className="idb-stat-value">{stats.duplicate}</span>
          <span className="idb-stat-label">🚫 Duplicate</span>
        </div>
      </div>

      {/* ── Filters ── */}
      <div className="idb-filters">
        <div className="idb-filter-group">
          <button 
            className={`idb-filter-btn ${filter === 'all' ? 'active' : ''}`}
            onClick={() => setFilter('all')}
          >
            All ({stats.total})
          </button>
          <button 
            className={`idb-filter-btn ${filter === 'nid' ? 'active' : ''}`}
            onClick={() => setFilter('nid')}
          >
            🪪 NID ({stats.nid})
          </button>
          <button 
            className={`idb-filter-btn ${filter === 'birth_certificate' ? 'active' : ''}`}
            onClick={() => setFilter('birth_certificate')}
          >
            📄 Birth ({stats.birth})
          </button>
          <button 
            className={`idb-filter-btn ${filter === 'passport' ? 'active' : ''}`}
        ...
        <div className="idb-filter-group">
          <button 
            className={`idb-filter-btn ${statusFilter === 'all' ? 'active' : ''}`}
            onClick={() => setStatusFilter('all')}
          >
            All Status
          </button>
          <button 
            className={`idb-filter-btn ${statusFilter === 'pending' ? 'active' : ''}`}
            onClick={() => setStatusFilter('pending')}
          >
            ⏳ Pending ({stats.pending})
          </button>
          <button 
            className={`idb-filter-btn ${statusFilter === 'approved' ? 'active' : ''}`}
            onClick={() => setStatusFilter('approved')}
          >
            ✅ Approved ({stats.approved})
          </button>
          <button 
            className={`idb-filter-btn ${statusFilter === 'rejected' ? 'active' : ''}`}
            onClick={() => setStatusFilter('rejected')}
          >
            ❌ Rejected ({stats.rejected})
          </button>
          <button 
            className={`idb-filter-btn ${statusFilter === 'duplicate' ? 'active' : ''}`}
            onClick={() => setStatusFilter('duplicate')}
          >
            🚫 Duplicate ({stats.duplicate})
          </button>
          <button 
            className={`idb-filter-btn ${statusFilter === 'auto' ? 'active' : ''}`}
            onClick={() => setStatusFilter('auto')}
          >
            🤖 Auto ({stats.autoAdded})
          </button>
          <button 
            className={`idb-filter-btn ${statusFilter === 'manual' ? 'active' : ''}`}
        ...
        <div className="idb-search">
          ...
          />
          {searchTerm && (
            <button className="idb-search-clear" onClick={() => setSearchTerm('')}>
              <i className="fa-solid fa-xmark"></i>
            </button>
          )}
        </div>
      ...
      {selectedRecords.length > 0 && (
        <div className="idb-bulk-actions">
          <span>{selectedRecords.length} selected</span>
          <button className="idb-bulk-delete" onClick={handleBulkDelete}>
            <i className="fa-solid fa-trash"></i> Delete Selected
          </button>
          <button className="idb-bulk-clear" onClick={() => setSelectedRecords([])}>
            Clear Selection
          </button>
        </div>
      )}
      ...
      {/* ── Table ── */}
      <div className="idb-table-wrapper">
        <table className="idb-table">
          ...
          <tbody>
            {filteredRecords.length === 0 ? (
              <tr>
                <td colSpan="8" className="idb-empty">
                  <i className="fa-solid fa-database"></i>
                  <p>No records found</p>
                  <button className="idb-btn-add-small" onClick={() => { setEditingRecord(null); setShowForm(true); }}>
              ...
              filteredRecords.map((record) => (
                <tr key={record.id} className={selectedRecords.includes(record.id) ? 'selected' : ''}>
                  ...
                  <td>
                    <div className="idb-user-info">
                      <div className="idb-user-name">{record.fullName}</div>
                      {record.address && (
                        <div className="idb-user-address">{record.address}</div>
                      )}
                      {record.autoAdded && (
                        <span className="idb-auto-badge">
                  ...
                  <td>
                    <span className={`idb-type-badge ${record.identityType}`}>
                      {record.identityType === 'nid' ? '🪪 NID' : 
                       record.identityType === 'birth_certificate' ? '📄 Birth' : '🛂 Passport'}
                    </span>
                  </td>
                  <td>
                    <span className="idb-id-number">{record.identityNumber}</span>
                  </td>
                  <td>
                    {record.phone && <div className="idb-contact">{record.phone}</div>}
                    {record.email && <div className="idb-contact small">{record.email}</div>}
                  </td>
                  <td>
                    {record.documentFront && (
                      <button 
                        className="idb-doc-btn"
                    ...
                    {record.documentBack && (
                      <button 
                        className="idb-doc-btn secondary"
                  ...
                  <td>
                    <span className={`idb-status ${record.status || 'pending'}`}>
                  ...
                  <td>
                    <div className="idb-actions">
                      ...
                      <button 
                        className="idb-action-delete" 
                        onClick={() => {
                          if (window.confirm(`"${record.fullName}" ডিলিট করতে চান?`)) {
      ...
      <div className="idb-footer">
        <span>
          ...
          {stats.autoAdded > 0 && (
            <span className="idb-footer-info">
              • 🤖 {stats.autoAdded} auto-saved from registration
            </span>
          )}
        </span>
        <button className="idb-btn-refresh" onClick={loadRecords}>
