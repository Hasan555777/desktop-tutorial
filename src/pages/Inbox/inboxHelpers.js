...
export const createNormalizedChatObject = (chat, currentUser) => {
  ...
  return {
    ...
    // ব্লক স্ট্যাটাস
    // ⚠️ NOT FIXED — flagging only: this is true only when the CURRENT
    // user is the one who did the blocking ("I blocked them"), not when
    // the other party blocked ME. BlockedView.jsx already works around
    // this with its own `blockedBy !== currentUser?.uid` check, but if
    // InboxChatItem.jsx reads `chat.isBlocked` directly for a list-level
    // lock icon, "they blocked me" would be invisible there. I don't have
    // InboxChatItem.jsx to confirm — send it if you want this checked.
