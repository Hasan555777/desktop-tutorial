...
// 5. saveFCMToken(): fallback setDoc now only runs for a 'not-found'
//    error, not blindly for any error -> avoids masking real errors
//    (permission-denied, network) with a pointless second write
//    attempt. Fallback also now uses serverTimestamp() to match the
//    primary path's timestamp type.
// 6. getFCMTokenFromDevice(): now reuses getFCMServiceWorkerRegistration()
//    instead of duplicating the raw register() call, so unsupported
//    browsers get a clear error instead of an obscure TypeError.
// 7. sendNotification(): retry loop now breaks early on non-retryable
...
export const saveFCMToken = async (userId, token) => {
  ...
  try {
    ...
    // 🔧 FIX: updateDoc only fails with 'not-found' when the user doc
    // doesn't exist yet. Any other error (permission-denied, network,
    // etc.) should NOT trigger a second write attempt — that just
    // masks the real problem and wastes a round trip.
    ...
    try {
      ...
      await setDoc(userRef, {
        notification: {
          token: token,
          platform: 'web',
          updatedAt: serverTimestamp(), // 🔧 FIX: match primary path's timestamp type
          enabled: true,
        }
      }, { merge: true });
...
export const getFCMTokenFromDevice = async () => {
  try {
    ...
    // 🔧 FIX: reuse the shared helper (which checks `serviceWorker in
    // navigator` and throws a clear error) instead of duplicating the
    // raw register() call here.
