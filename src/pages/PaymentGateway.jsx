...
//
// ⚠️ ডিল কখনোই এখানে 'completed' মার্ক করা হয় না। সব মাইলস্টোন funded হলেও
...
// number depending on how the deal was created — never assume it's numeric.
// URL params (useParams) are ALWAYS strings, so always compare as strings.
const findMilestone = (milestones, milestoneId) =>
  (milestones || []).find(m => String(m.id) === String(milestoneId));
...
const PaymentGateway = () => {
    ...
    const loadData = async () => {
      try {
        ...
        if (dealSnap.exists()) {
          ...
          // ✅ Load sender balance + locked balance (Buyer)
          ...
          if (foundMilestone) {
            ...
            // ✅ Already funded/released — nothing to pay here anymore.
  ...
  const handleSendMoneyPayment = async () => {
    ...
    try {
      ...
      await runTransaction(db, async (transaction) => {
        ...
        // 3. Update Buyer wallet (DEBIT the real balance, and release this
        ...
        // 4. Seller wallet: NOT credited here. Money sits in escrow
        ...
        // 5. ✅ Buyer transaction record (escrow debit — not a transfer to
        // the seller yet, so it's labeled clearly as "Escrow Funded").
        ...
        // 6. Update Milestone status — using the FRESH milestones array
        // (read inside the transaction) and string-safe id comparison.
        ...
        // ⚠️ FIXED BUG: the deal must NEVER be marked 'completed' here.
        // "All milestones funded" only means all the money is sitting in
        // escrow — the seller hasn't been paid and the work may not even
        // be done yet. 'completed' is set exclusively in
        // DealManager.jsx's handleReleasePayment, once every milestone
        // reaches 'released'.
      ...
      try {
        // To BUYER
        await sendDealPaymentNotification(
          user.uid,
          amount,
          dealData.postTitle || 'Deal Payment',
          dealId,
          'funded'
        );

        // To SELLER — let them know the milestone is funded and they can
        // now start/submit work.
  ...
  // ✅ Render
  // ============================================================
  const availableBalance = senderBalance; // balance already reflects real spendable escrow-fundable amount
  const totalAvailableIncludingLocked = senderBalance; // funding checks against real balance, not lockedBalance
    ...
    <div className="payment-container">
      <div className="payment-card">
        ...
        <div className="payment-header">
          ...
          <h2><i className="fa-solid fa-credit-card"></i> Fund Milestone</h2>
          {isBuyer ? (
        ...
        <div className="payment-details">
          ...
          <div className="detail-row">
            ...
            <strong style={{ color: senderBalance >= (milestone?.amount || 0) ? '#10b981' : '#ef4444' }}>
              ...
              {senderBalance < (milestone?.amount || 0) && (
        ...
        {paymentStep === 1 && isBuyer && (
          <div className="payment-options">
            <button
              className="pay-option-btn send-money"
              onClick={handleSendMoneyPayment}
              disabled={processing || senderBalance < (milestone?.amount || 0)}
            >
            ...
            <button
              className="pay-option-btn deposit"
        ...
        {paymentStep === 1 && !isBuyer && (
          <div className="payment-not-authorized">
            ...
            <p>You are the <strong>Seller</strong>. Only the <strong>Buyer</strong> can fund this milestone.</p>
            <p style={{ fontSize: '13px', color: 'var(--text-muted)' }}>
              Please wait for the Buyer to complete the payment before you start work.
            </p>
          </div>
        )}
