...
// styles কম্পোনেন্টের বাইরে module-level constant — এতে প্রতি রেন্ডারে
// নতুন অবজেক্ট তৈরি হয় না (postData-নির্ভর কিছু নেই এতে)
const styles = {
  card: {
    padding: '12px 16px',
  ...
  inner: { display: 'flex', gap: '12px', alignItems: 'flex-start' },
  image: {
  ...
  title: {
    ...
    maxHeight: '2.8em', marginBottom: '4px', wordBreak: 'break-word'
  },
  description: {
    fontSize: '12px', color: '#6b7280',
    display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical',
    overflow: 'hidden', textOverflow: 'ellipsis', lineHeight: '1.5',
    maxHeight: '3em', marginBottom: '8px', wordBreak: 'break-word'
  },
  meta: { display: 'flex', gap: '12px', fontSize: '12px', color: '#4b5563', marginBottom: '8px', flexWrap: 'wrap' },
  metaItem: { display: 'flex', alignItems: 'center', gap: '4px' },
  metaIcon: { marginRight: '4px' },
  ...
  buttonView: { backgroundColor: '#f3f4f6', border: '1px solid #d1d5db', color: '#374151' },
  buttonOffer: { backgroundColor: '#3b82f6', color: 'white' },
  buttonApprove: { backgroundColor: '#10b981', color: 'white' },
  buttonReject: { backgroundColor: '#ef4444', color: 'white' },
  buttonReopen: { backgroundColor: '#8b5cf6', color: 'white' },
  badge: { padding: '4px 12px', fontSize: '12px', borderRadius: '6px', display: 'flex', alignItems: 'center', gap: '4px', fontWeight: '500' },
  ...
  badgeCancelled: { backgroundColor: '#fef2f2', color: '#991b1b' }
};
    ...
    <div style={styles.card}>
      <div style={styles.inner}>
        ...
        <div style={styles.info}>
          <div style={styles.title} title={fullTitle || title || 'No title'}>
            {title || 'No title'}
          </div>

          <div style={styles.description} title={fullDescription || description || 'No description'}>
          ...
          <div style={styles.actions}>
            <button style={{ ...styles.button, ...styles.buttonView }} onClick={onViewPost}>
              <i className="fa-regular fa-eye"></i> View Post
            </button>

            {canSendOffer && (
              <button style={{ ...styles.button, ...styles.buttonOffer }} onClick={onSendOffer}>
            ...
            {canApproveDeal && (
              <>
                <button style={{ ...styles.button, ...styles.buttonApprove }} onClick={onApproveDeal}>
                  <i className="fa-solid fa-check-circle"></i> Confirm Deal
                </button>
                <button style={{ ...styles.button, ...styles.buttonReject }} onClick={onRejectDeal}>
            ...
            {existingDeal?.status === 'cancelled' && (
              <>
                ...
                <button style={{ ...styles.button, ...styles.buttonReopen }} onClick={onReopenDeal}>
