...
// Logic is unchanged from the original DealManager.jsx.
...
export const useDealActions = ({
  ...
  const handleReleasePayment = async (milestoneId) => {
    ...
    try {
      ...
      const updatedMilestones = selectedDeal.milestones.map((m) =>
        m.id === milestoneId
          ? { ...m, status: 'released', releasedAt: new Date().toISOString(), releasedBy: currentUser?.uid }
          : m
      );
      ...
      if (allReleased) {
        notifyBothParties(notification, NOTIFICATION_EVENTS.DEAL_COMPLETED, parties(), {
          dealId: selectedDeal.id,
          postTitle: selectedDeal.postTitle || 'Untitled Deal',
        });
  ...
  const handleConfirmDeal = async () => {
    ...
    try {
      ...
      notifyBothParties(notification, NOTIFICATION_EVENTS.DEAL_CONFIRMED, parties(), {
        dealId: selectedDeal.id,
        postTitle: selectedDeal.postTitle || 'Untitled Deal',
  ...
  const handleCancelDeal = async () => {
    ...
    const hasPayment = selectedDeal.milestones?.some(
      (m) => m.status === 'funded' || m.status === 'released' || m.status === 'review'
    );
    if (hasPayment) {
    ...
    if (response === 'approve') {
      try {
        ...
        if (selectedDeal.cancelRequestStatus === 'pending') {
          ...
          const isOtherParty =
            (isBuyer && selectedDeal.buyerId === currentUser?.uid) ||
          ...
          notifyBothParties(notification, NOTIFICATION_EVENTS.CANCELLATION_APPROVED, parties(), {
            dealId: selectedDeal.id,
            postTitle: dealTitle,
          });
