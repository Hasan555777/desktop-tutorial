...
const AppLockScreen = ({ onUnlock }) => {
  ...
  const handleSendRecoveryOTP = useCallback(async () => {
    if (!recoveryEmail || !recoveryEmail.includes('@')) {
      feedback?.showWarning('⚠️ Invalid Email', 'Please enter a valid email address.');
      return;
    }
    ...
    try {
      ...
      feedback?.showSuccess('✅ OTP Sent', 'Check your email for recovery code.');
      setShowRecoveryInput(true);
    } catch (error) {
      console.error('❌ Recovery error:', error);
      feedback?.showError('❌ Failed', 'Could not send recovery code. Try again.');
    } finally {
      setIsRecoveryLoading(false);
    }
  }, [recoveryEmail, feedback]);
  ...
  const renderForgotPinModal = () => {
    ...
    return (
      <div className="forgot-pin-overlay" onClick={() => setShowForgotPin(false)}>
        <div className="forgot-pin-modal" onClick={(e) => e.stopPropagation()}>
          <h3>
            ...
            <>
              <div className="form-group">
                ...
                <small style={{ color: 'var(--text-muted)', fontSize: '12px', marginTop: '4px' }}>
                  Format: 4 characters + 4 characters (e.g., A8KF-92LM)
                </small>
              </div>
          ...
          {!showRecoveryInput && !showPinReset && emergencyUnlock && (
            <div className="emergency-unlock-section">
              ...
              <div className="emergency-info">
                ...
                <div>
                  <p className="emergency-title">অ্যাডমিন ইমার্জেন্সি আনলক উপলব্ধ</p>
  ...
  return (
    <div className="app-lock-screen">
      ...
      <div className="lock-content">
        {/* ── App Icon ── */}
        <div className="lock-app-icon">
          <i className="fa-solid fa-cube"></i>
        </div>

        {/* ── Title ── */}
        <h2 className="lock-title">🔐 App Lock</h2>
