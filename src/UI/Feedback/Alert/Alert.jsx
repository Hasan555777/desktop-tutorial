...
const Alert = React.memo(({
  ...
  return (
    <Modal
      ...
      size="sm"
    >
      <div className={`alert-container ${variantClass}`}>
        {/* Icon */}
        <div className="alert-icon-wrapper">
          <div className="cube-background">
            <i className="fa-solid fa-cube"></i>
          </div>
          <div className={`alert-status-icon ${variantClass}`}>
        ...
        {/* Status Label */}
        {statusLabel && (
          <div className={`alert-status-label ${variantClass}`}>
            {statusLabel}
          </div>
        )}

        {/* Title */}
        <h3 id={labelledBy} className="alert-title">
          {finalTitle}
        </h3>

        {/* Message */}
        {finalMessage && (
          <p id={describedBy} className="alert-message">
            {finalMessage}
          </p>
        )}
        ...
        <button
          ref={okButtonRef}
          className={`alert-btn ${variantClass}`}
