...
import { SOUND_EVENTS } from '../Sound/SoundEvents';
...
import { getCategoryForEvent, isCategoryEnabled, isSoundCategoryEnabled, isChatMessagingEvent } from '../../../features/notifications/utils/notificationCategory';
import { isAdminUser } from '../../../features/admin/constants/admin';
...
export const NotificationProvider = ({ children }) => {
    ...
    ({
      ...
      try {
        ...
        notification.onclick = () => {
          window.focus();
          // 🔧 FIX (item #7 investigation): this used to set
          // window.location.href / window.location.pathname for
          // in-app paths too, forcing a full page reload instead of
          // client-side routing. A full reload resets the SPA's
          // history stack — anything the user navigated through
          // before clicking the notification is gone, and the fresh
          // reload can briefly redirect through PrivateRoute while
          // auth re-initializes, inserting an extra history entry.
          // That's a very plausible source of "back button jumps to
          // home unexpectedly." External URLs still need a real
          // browser navigation; only in-app paths switch to navigate().
          if (clickUrl) {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(
      ...
      (snapshot) => {
        ...
        // 🔧 FIX (#9 unified unread count): chat messages get their
        // own CHAT_MESSAGE/CHAT_IMAGE/etc. doc here too (for the
        // toast/sound below), but Notifications.jsx hides those from
        // its list and nothing ever marks them read — markChatAsRead
        // (useChatMessages.js) only resets the `chats/{id}.unreadCount`
        // map, never these docs' `isUnread`. Counting them here made
        // this badge an ever-growing, inconsistent SECOND unread
        // counter for the same messages. The `chats` collection is
        // already the reliable source of truth for chat unread —
        // App.jsx adds that total to this `unreadCount` when showing
        // the combined bell badge, so excluding chat events here
        // avoids double-counting instead of dropping them entirely.
        const unread = snapshot.docs.filter((d) => {
          const data = d.data();
          if (isChatMessagingEvent(data.event)) return false;
  ...
  useEffect(() => {
    ...
    if (!isAdminUser(user, userRole, userData?.adminDisabled)) {
      setAdminUnreadCount(0);
      return;
    }
  ...
  }, [currentUser, userRole, userData?.adminDisabled]);
