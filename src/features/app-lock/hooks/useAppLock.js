...
import {
  ...
  SESSION_KEY,
  DEVICE_KEY,
} from '../../../shared/security/constants';
import { hashPin, verifyPin as verifyPinHash, generateSalt, generateSecureToken } from '../../../shared/security/crypto';
    ...
    try {
      ...
      if (userDoc.exists()) {
        ...
        if (appLock?.enabled) {
          ...
          // 🔧 FIX: this used to only set hasPin=true when Firestore had a
          // pinHash — it never actually copied pinHash/pinSalt into local
          // storage. verifyPin() reads the hash from LOCAL storage only,
          // so on a second browser/device, or after localStorage was
          // cleared, the app showed "PIN is set" (from this Firestore
          // check) but verifyPin() would fail with "No PIN set" — the app
          // lock silently stopped working the moment storage didn't have
          // a local copy. Hydrating local storage from Firestore here
          // makes the PIN work consistently across devices/storage-clears.
          if (appLock.pinHash) {
            setHasPin(true);
            const localPin = storage.get(PIN_HASH_KEY);
            if (!localPin?.hash) {
              storage.set(PIN_HASH_KEY, {
                hash: appLock.pinHash,
                salt: appLock.pinSalt,
                iterations: appLock.iterations,
                algorithm: appLock.algorithm,
                length: appLock.pinLength || DEFAULT_PIN_LENGTH,
              });
              setPinLength(appLock.pinLength || DEFAULT_PIN_LENGTH);
            }
          }

          // 🔧 FIX: an active lockout (lockedUntil) IS written to
          // Firestore when a user hits MAX_PIN_ATTEMPTS, but this
          // function never read it back — only local storage's lockout
          // state was ever applied. Clearing localStorage, or opening a
          // different browser/device, reset the lockout to "not locked"
          // regardless of what Firestore said, letting someone bypass a
          // 5-minute lockout just by clearing site data. Restoring it
          // here closes that gap.
          if (appLock.lockedUntil) {
            const lockedUntilMs =
              typeof appLock.lockedUntil?.toMillis === 'function'
                ? appLock.lockedUntil.toMillis()
                : Number(appLock.lockedUntil);
            if (lockedUntilMs && lockedUntilMs > Date.now()) {
              setIsLockedOut(true);
              setLockedUntil(new Date(lockedUntilMs));
              setAttempts(appLock.attempts || 0);
              setRemainingAttempts(Math.max(0, MAX_PIN_ATTEMPTS - (appLock.attempts || 0)));
