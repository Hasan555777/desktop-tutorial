...
export const LIVENESS_STEPS = [
  { key: 'blink', label: 'চোখ টিপুন/অপেক্ষা করুন  ', emoji: '👁️' },
];
...
// ============================================================
// ✅ Cloudinary Upload
// 🔧 FIX (item #5): compression used to be opt-in per call-site — several
// upload paths across the app (profile photo, NID/birth cert documents,
// deposit receipts, face verification, ProfileHeader) called this without
// compressing first, so full-resolution camera photos went straight to
// Cloudinary. Compression now happens HERE, automatically, so no caller
// can forget it. PDFs and HEIC/HEIF are skipped — <canvas>/<img> can't
// decode either in-browser, so compressing them would just fail; they
// upload as-is (PDF documents are already compact; HEIC/HEIF only occurs
// for the general image path, not documents, per ALLOWED_IMAGE_TYPES).
// ============================================================
const COMPRESSIBLE_TYPES = ['image/jpeg', 'image/png', 'image/webp'];

// 🔧 FIX (#7 — document upload sometimes fails): the Cloudinary POST had
// no timeout and no retry. A slow/dropped connection left the caller
// awaiting forever (same class of "stuck" bug as the OTP requests), and
// any transient network blip (very common on mobile data) failed the
// whole upload permanently with no chance to recover automatically.
// Added a request timeout plus a single automatic retry for exactly the
// failure modes that are actually transient (timeout / network error) —
// a real validation or server-rejection error still fails immediately,
// since retrying those would never succeed anyway.
...
export const uploadToCloudinary = async (file, folder = 'user_documents', { _isRetry = false } = {}) => {
  try {
    // ✅ ফাইল ভ্যালিডেশন
    // deposit_receipts uses the document profile too — receipts need to
    // stay legible (bank transaction text/numbers), so they get the
    // larger 1600x1200 @0.85 treatment instead of the tighter face-photo one.
    const isDocument = folder === 'nid_documents' || folder === 'birth_documents' || folder === 'deposit_receipts';
    ...
    if (COMPRESSIBLE_TYPES.includes(file.type)) {
      try {
        fileToUpload = isDocument
          ? await compressDocument(file)
          : await compressFacePhoto(file);
      } catch (compressionError) {
        // Compression is a best-effort optimization — if it fails for any
        // reason, fall back to uploading the original rather than blocking
        // the whole upload.
    ...
    const res = await fetch(
      ...
      { method: 'POST', body: fd, signal: AbortSignal.timeout(CLOUDINARY_TIMEOUT_MS) }
    );
    ...
    if (!_isRetry && isTransientUploadError(error)) {
      console.warn('↻ Retrying upload once after a transient network/timeout error...');
      return uploadToCloudinary(file, folder, { _isRetry: true });
    }
    throw error;
  }
};
