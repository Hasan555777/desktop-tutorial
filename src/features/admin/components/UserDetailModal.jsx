...
const UserDetailModal = ({ 
  ...
  // ── Handle Save Review ──
  // 🔧 FIX: this used to fire-and-forget `onSaveReview(...)` and never
  // close the modal — the save itself worked fine, but nothing ever told
  // the modal it was done. Now we await the result and only close on a
  // confirmed success (`true`); a permission-denied or failed save
  // resolves falsy, so the modal stays open (the admin already sees the
  // relevant error toast) instead of silently discarding their review.
  ...
  const handleSaveReview = async () => {
    ...
    try {
      const success = await onSaveReview(user.id, faceReview, documentReview);
      if (success) onClose();
    } finally {
