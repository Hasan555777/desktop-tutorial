...
import { emergencyUnlockUser, setTemporaryPassword } from '../firebase/adminFunctions';
...
const UsersTable = ({ 
  ...
  // ============================================================
  // 🔐 Set Temporary Password Handler
  // ============================================================
  const handleSetTempPassword = async (userId, userEmail) => {
    const confirmed = await feedback.confirm({
      title: '🔐 সাময়িক পাসওয়ার্ড সেট করুন',
      message: `এই ইউজারের (${userEmail || userId}) জন্য একটি নতুন সাময়িক পাসওয়ার্ড সেট করা হবে। ইউজার এই পাসওয়ার্ড দিয়ে লগইন করে সেটিংস → সিকিউরিটি ট্যাব থেকে নিজের নতুন পাসওয়ার্ড সেট করতে বাধ্য থাকবে। এগিয়ে যাবেন?`,
      variant: 'warning',
      confirmText: 'হ্যাঁ, সেট করুন',
      cancelText: 'বাতিল করুন'
    });
    ...
    try {
      const result = await setTemporaryPassword(userId);
      if (result.success) {
        feedback.alert.success({
          title: '✅ সাময়িক পাসওয়ার্ড সেট হয়েছে',
          message: `পাসওয়ার্ড: ${result.tempPassword}\n\nএই পাসওয়ার্ডটি নিরাপদ উপায়ে ইউজারকে জানান — এটি আর কোথাও দেখানো হবে না। ইউজার লগইন করলে স্বয়ংক্রিয়ভাবে পাসওয়ার্ড পরিবর্তনের পেজে চলে যাবে।`,
        });
      } else {
        feedback.showError('❌ ব্যর্থ', result.error || 'সাময়িক পাসওয়ার্ড সেট করতে সমস্যা হয়েছে');
      }
    } catch (error) {
      console.error('Set temp password error:', error);
      feedback.showError('❌ ব্যর্থ', 'সাময়িক পাসওয়ার্ড সেট করতে সমস্যা হয়েছে');
    }
  };
              ...
              <tr key={user.id} className={isBlocked ? styles.banned : ''}>
                ...
                <td>
                  <div className={styles.actionButtons}>
                    ...
                    <button 
                      className={`${styles.actionBtn} ${styles.emergencyUnlock}`} 
                      onClick={() => handleSetTempPassword(user.id, user.email)}
                      title="সাময়িক পাসওয়ার্ড সেট করুন (পাসওয়ার্ড ভুলে গেলে)"
                    >
                      🔐
                    </button>
                  </div>
