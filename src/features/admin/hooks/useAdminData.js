...
import { auth, db, functions } from '../../../shared/firebase/index';
...
const deleteIdentityRecord = useCallback(async (recordId) => {
  ...
  const saveVerificationReview = useCallback(async (userId, faceReview, documentReview) => {
    try {
      ...
      return true; // 🔧 FIX: let the caller (modal) know the save actually
      // succeeded, so it knows it's safe to close itself. Previously this
      // returned nothing either way, so "Save Review" had no way to tell
      // success from failure and never closed the modal.
      ...
      return false; // 🔧 FIX: save failed — caller should keep the modal
      // open so the admin can see the error and retry, not lose their review.
    }
  }, [feedback, reloadAllData, createUserNotification]);
...
// ============================================================
// 📌 EDIT POST (Admin - Pending Posts) - UPDATED
// ============================================================

const handleEditPost = useCallback(async (postId, formData) => {
  try {
    // ✅ সম্পূর্ণ পোস্ট ডেটা প্রস্তুত করুন
    const updatedData = {
      title: formData.title,
      description: formData.description,
      budget: typeof formData.budget === 'object' ? formData.budget : {
        amount: Number(formData.budget || 0),
        type: 'fixed',
        isNegotiable: false
      },
      deadline: typeof formData.deadline === 'object' ? formData.deadline : {
        days: Number(formData.deadline || 0),
        type: 'fixed'
      },
      updatedAt: serverTimestamp(),
    ...
    // ✅ পোস্ট অ্যাপ্রুভ করুন (স্টেটাস পরিবর্তন)
    await updateDoc(postRef, {
      status: 'approved',
      approvedAt: serverTimestamp(),
      approvedBy: auth.currentUser?.uid || 'admin'
    ...
    if (post?.userId) {
      await createUserNotification(
        post.userId,
        `✅ আপনার পোস্ট "${formData.title}" এডিট এবং অ্যাপ্রুভ করা হয়েছে!`,
        'success'
      );
    }

    // ✅ লোকাল স্টেট আপডেট - পেন্ডিং পোস্ট থেকে রিমুভ করুন
    setPendingPosts(prev => prev.filter(p => p.id !== postId));

    // ✅ পোস্ট লিস্টে যোগ করুন
    setPosts(prev => {
      const updatedPost = { 
        ...post, 
        ...updatedData, 
    ...
    await feedback.showSuccess('✅ পোস্ট আপডেট এবং অ্যাপ্রুভ', 'পোস্ট সফলভাবে আপডেট এবং অ্যাপ্রুভ করা হয়েছে!');
    await reloadAllData();
...
}, [feedback, reloadAllData, pendingPosts, createUserNotification]);
...
// ============================================================
// 📌 EDIT PENDING EDIT (Admin - Pending Edits) - UPDATED
// ============================================================

const handleEditPendingEdit = useCallback(async (editId, formData) => {
  try {
    // ✅ সম্পূর্ণ পেন্ডিং চেঞ্জেস ডেটা প্রস্তুত করুন
    ...
    await updateDoc(postRef, {
      'pendingChanges.title': updatedPendingChanges.title,
      'pendingChanges.description': updatedPendingChanges.description,
      'pendingChanges.budget': updatedPendingChanges.budget,
      'pendingChanges.deadline': updatedPendingChanges.deadline,
      updatedAt: serverTimestamp(),
      lastEditedBy: auth.currentUser?.uid || 'admin',
    });
    ...
    setPendingEdits(prev => prev.map(edit => 
      edit.id === editId ? { 
        ...
        pendingChanges: {
          ...edit.pendingChanges,
          title: updatedPendingChanges.title,
          description: updatedPendingChanges.description,
          budget: updatedPendingChanges.budget,
          deadline: updatedPendingChanges.deadline,
        }
      } : edit
    ));
