...
import { isCategoryEnabled, isChatMessagingEvent } from './utils/notificationCategory';
...
const Notifications = ({ currentUser }) => {
  ...
  // 🔧 FIX (#9): now shared with NotificationProvider.jsx (see
  // notificationCategory.js) so both places agree on which events are
  // "belongs to chat, shown via the chat unread count" vs a general
  // notification — one definition instead of two that could drift.
  const isChatEvent = (event) => isChatMessagingEvent(event);
    ...
    <div className={styles.notificationsContainerWrapper}>
      <div className={styles.notificationsPanel}>
        ...
        <div className={styles.notiHeader}>
          <div className={styles.notiHeaderLeft}>
            <div className={styles.notiHeaderIcon}>
              <i className="fa-solid fa-bell"></i>
              {unreadCount > 0 && <span className={styles.notiHeaderPulse}></span>}
            </div>
            <div className={styles.notiHeaderInfo}>
              <h2>Notifications</h2>
            </div>
          </div>
          
          <div className={styles.notiHeaderRight}>
            {unreadCount > 0 && (
            ...
            {notifications.length > 0 && (
              <button className={styles.clearAllBtn} onClick={handleClearAllNotifications}>
        ...
        <div className={styles.notiListBody}>
          {notifications.length > 0 ? (
            notifications.map((noti) => (
              <div 
                key={noti.id} 
                className={`${styles.notiItemCard} ${noti.isUnread ? styles.unreadNoti : styles.readNoti}`}
                onClick={() => handleNotificationClick(noti)}
              >
                <div className={`${styles.notiIconBox} ${styles[noti.colorClass || 'notiSystem']}`}>
                  <i className={noti.icon || 'fa-solid fa-bell'}></i>
                </div>

                <div className={styles.notiContentBlock}>
                  <div className={styles.notiTopLine}>
                    <h4>{noti.title || 'Notification'}</h4>
                    <span className={styles.notiTime}>{noti.time}</span>
                  </div>
                  <p className={styles.notiMessage}>{noti.message}</p>
                  {noti.actionRequired && (
                    <div className={styles.notiActionBadge}>
                ...
                <button 
                  className={styles.notiDeleteBtn}
                ...
                {noti.isUnread && <div className={styles.notiUnreadDot}></div>}
              </div>
            ))
          ) : (
            <div className={styles.noNotiNotice}>
