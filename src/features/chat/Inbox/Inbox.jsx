...
import { useUserProfiles } from '../../profile/hooks/useUserProfiles';
      ...
      <div className={styles.workspaceInboxWrapper}>
        <div className={styles.workspaceInbox}>
          ...
          <div className={styles.inboxRowsContainer}>
            ...
            {/* 🔧 FIX (Inbox #10, continued): brief skeleton for the
                narrow window after chats have loaded but before the
                referenced users' profiles have resolved (isDataReady
                still false). Previously this gap didn't exist because
                isDataReady flipped true too early — now that it
                correctly waits for profiles too, this window is much
                shorter, but still worth an explicit loading state
                instead of blank space per the "correct data or
                intentional loading state, never incorrect temporary
                data" requirement. */}
            {!loading && !isDataReady && filteredChats.length > 0 && (
              <div className={styles.inboxSkeletonList} aria-hidden="true">
                {[0, 1, 2].map(i => (
                  <div key={i} className={styles.inboxSkeletonRow}>
                    <div className={styles.inboxSkeletonAvatar} />
                    <div className={styles.inboxSkeletonLines}>
                      <div className={styles.inboxSkeletonLine} style={{ width: '55%' }} />
                      <div className={styles.inboxSkeletonLine} style={{ width: '80%' }} />
                    </div>
                  </div>
                ))}
              </div>
              ...
              return (
                <div key={chat.id} style={{ position: 'relative' }}>
                  ...
                  <InboxChatMenu
                    ...
                    hasActiveDeal={activeDeal.hasActiveDeal}
                    activeDealCount={activeDeal.count}
                    otherUserId={chat.otherPartyId || chat.participants?.find(p => p !== currentUser?.uid)}
                  />
                </div>
              );
            })}
