...
import { createNewChatObject, createNormalizedChatObject, getOtherPartyIdFromContext, getDeterministicChatId } from '../inboxHelpers';
import { useAuth } from '../../../../shared/context/AuthContext';
import { useChatActions } from '../../hooks/useChatActions';
import { checkActiveDealBetweenUsers } from '../../chatHelpers';
...
export const useInboxChats = (currentUser, currentMode, selectedChat, setSelectedChat, fetchUserProfile, chatContext) => {
      ...
      async (snapshot) => {
        ...
        try {
          ...
          if (isMounted.current) {
            ...
            // 🔧 FIX (Inbox #10): isDataReady used to be set true right
            // here, before the user-profile fetches below even started.
            // The chat list renders as soon as isDataReady flips, using
            // chat docs that reference otherPartyId/buyerId/sellerId —
            // but userProfiles cache entries for those IDs were still
            // empty at that moment, so InboxChatItem briefly rendered
            // with no name/avatar/online-status (or a generic
            // fallback) until fetchUserProfile resolved a couple
            // seconds later. Moved below, after the profile fetches
            // actually complete, so the list only renders once both
            // chats AND the identities they reference are ready.
          }
  ...
  useEffect(() => {
    ...
    // 🔧 FIX (chat ID collision bug): see the long comment in
    // inboxHelpers.js. Must compute the SAME pair-based ID here as
    // createNewChatObject does, so the "does this chat already
    // exist" check below and the doc ref we might write to both
    // agree with what actually gets saved.
    const otherPartyId = getOtherPartyIdFromContext(chatContext);
    const chatId = getDeterministicChatId(currentUser.uid, otherPartyId) ||
      chatContext.id || chatContext.postId;
    if (!chatId) return;
    ...
    if (!existingChat) {
      ...
      // 🔧 merge: true as defense-in-depth — if this ever races with
      // another tab/device already having created the same pair's
      // chat (e.g. right after isDataReady flips but before `chats`
      // has the freshest snapshot), this won't stomp its existing
      // fields the way a bare setDoc would.
      setDoc(chatRef, newChat, { merge: true }).catch(error => logger.error("Error creating chat:", error));
