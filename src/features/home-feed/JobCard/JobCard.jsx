...
function JobCard({ 
  ...
  // ============================================================
 return (
    <div className={styles.jobPostCard} id={id || `post-${jobId}`}>
      {/* ── হেডার সেকশন ── */}
      <div className={styles.postTopMeta}>
        <Link 
          to={`/profile/user/${userId}`} 
          style={{ textDecoration: 'none', color: 'inherit', flex: 1, minWidth: 0 }}
          onClick={(e) => e.stopPropagation()}
        >
          <div className={styles.clientProfileBlock}>
            <div className={styles.clientAvatar}>
              {clientProfile.clientPhoto ? (
                <>
                  {!avatarLoaded && (
                    <div className={styles.jcSkeletonAvatar} aria-hidden="true"></div>
                  )}
                  <img 
                    src={avatarCacheBustedUrl} 
                    alt={clientProfile.clientName || 'User'} 
                    className={`${styles.clientAvatarImg} ${avatarLoaded ? styles.loaded : styles.hidden}`}
            ...
            <div className={styles.clientInfo}>
              <span className={styles.clientName} title={clientProfile.clientName}>
                {displayName || clientProfile.clientName || 'Unknown User'}
              </span>

              <div className={styles.clientRatingSection}>
                ...
                <div className={styles.ratingReviewsCount}>
                  {rating.total > 0 ? (
                    <span>
                      <i className="fa-regular fa-comment"></i> {rating.total} {rating.total === 1 ? 'Review' : 'Reviews'}
                    </span>
                  ) : (
                    <span className={styles.noReviews}>Be the first to review</span>
                  )}
                </div>
              </div>

              {clientProfile.verified && (
                <span className={styles.verifiedBadges}>
        ...
        <div className={styles.jcTopRightActions}>
          <div className={styles.jcActionIconGroup}>
            <button 
              className={`${styles.jcActionIconBtn} ${isSaved ? styles.saved : ''}`} 
            ...
            <button 
              className={styles.jcActionIconBtn} 
          ...
          <span className={styles.postTimeBadge}>
      ...
      <div className={styles.postMainContent}>
        <h3 className={styles.jobTitle} title={title || ''}>
          {displayTitle}
        </h3>
        
        <p className={styles.jcDynamicText}>
          {getDisplayText()}
          {shouldTruncate && (
            <button 
              className={styles.jcSeeMoreBtn} 
      ...
      {/* ── ইমেজ গ্যালারি ── */}
      {postImages.length > 0 && (
        <div className={`${styles.jcImageGalleryGrid} ${styles[`galleryCols${Math.min(postImages.length, 3)}`]}`}>
          {postImages.slice(0, 3).map((imgUrl, index) => (
            <div 
              key={`img-${jobId}-${index}`} 
              className={styles.jcGalleryImgWrapper} 
              onClick={() => setActiveZoomImage(imgUrl)}
            >
              {!imagesLoaded[index] && (
                <div className={styles.jcSkeletonImage} aria-hidden="true"></div>
              )}
              
              <img 
                src={imgUrl} 
                alt={`Job Attachment ${index + 1}`} 
                className={`${styles.jcFeedLiveImg} ${imagesLoaded[index] ? styles.loaded : styles.hidden}`}
                loading="lazy"
          ...
          {postImages.length > 3 && (
            <div className={`${styles.jcGalleryImgWrapper} ${styles.moreImages}`} onClick={() => setActiveZoomImage(postImages[3])}>
              <img 
                src={postImages[3]} 
                alt="More" 
                className={styles.jcFeedLiveImg} 
                onError={handleImageError}
              />
              <div className={styles.moreOverlay}>+{postImages.length - 3}</div>
            </div>
          )}
        </div>
      )}
      ...
      <JobLock
        ...
        className={styles.jobLockWrapper}
      />

      {/* ── Budget & Deadline Info Strip ── */}
      <div className={styles.postInfoStrip}>
        <div className={`${styles.stripItem} ${styles.siBudget}`}>
          <i className="fa-solid fa-wallet"></i>
          Budget: <strong>{budgetInfo.display}</strong>
        </div>
        <div className={`${styles.stripItem} ${styles.siDeadline}`}>
      ...
      <div className={styles.postFooterActions}>
        <button 
          className={`${styles.btnApplyJob} ${isPostLocked ? styles.locked : ''}`}
      ...
      {/* ── জুম লাইটবক্স ── */}
      {activeZoomImage && (
        <div className={styles.zoomLightboxOverlay} onClick={() => setActiveZoomImage(null)}>
          <button className={styles.lightboxCloseBtn} onClick={() => setActiveZoomImage(null)}>
