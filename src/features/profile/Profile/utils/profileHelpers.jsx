...
// 🔧 FIX (item #5): also compresses automatically now, same as
// registerHelpers.js — see that file for the full rationale. Every
// Profile-page upload (profile photo, NID/birth cert docs) now gets
// compressed without the caller having to remember to do it.
// ============================================================
const COMPRESSIBLE_TYPES = ['image/jpeg', 'image/png', 'image/webp'];

// 🔧 FIX (#7 — document upload sometimes fails): matches the same fix
// applied to registerHelpers.js's uploadToCloudinary — this copy had no
// request timeout and no retry either, so a stalled connection during a
// Profile-page document (re)upload could hang forever with no error, and
// any transient network blip failed the upload permanently with no
// automatic recovery.
...
export const uploadToCloudinary = async (file, folder = 'user_documents', { _isRetry = false } = {}) => {
  ...
  try {
    const response = await fetch(
      ...
      { method: 'POST', body: formData, signal: AbortSignal.timeout(CLOUDINARY_TIMEOUT_MS) }
    );
